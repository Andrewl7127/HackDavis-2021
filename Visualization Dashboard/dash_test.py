...
app.title = 'Live COVID-19 Dashboard'

app.layout = html.Div(children=[
    html.H1('Live COVID-19 Dashboard'),
    html.H4('Created by Adhvaith Vijay'),
    ...
    html.Div([
        ...
        dcc.Markdown('''
            * Run the animation and hover over map for state-specific information. Data is pulled from the 
              New York Times' GitHub repository, so this map updates in real-time.
            
            * The map will automatically be updated and display Avg. ROC information for later months in 2021
              as time goes by and the year progresses.
...
def get_clusters(n, user_input, rank_by):
    ...
    fig = px.choropleth_mapbox(subset, geojson=counties, locations='fips', color='Cluster',
                               ...
                               hover_name='county',
                               hover_data={'fips': False, 'Cluster': True, 'state': True},
                               mapbox_style="carto-positron",
                               zoom=3.2, center={"lat": 38.0902, "lon": -95.7129},
                               opacity=0.8,
                               labels={'state': 'State'}
